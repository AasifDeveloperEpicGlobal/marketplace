...
const UpdateSubscription = ({ setSubTab, currentId }: activeProps) => {
  ...
  const [payment_status, setPaymentStatus] = useState<boolean>(false);
  ...
  const [total, setTotal] = useState<string>(currentId?.Amount);
  const [subsPlan, setSubsPlan] = useState("services");
  const [selectGST, setSelctGST] = useState("no");
  const [selectedGSTValue, setSelectedGSTValue] = useState<string>("");
  const [gsttype, GstType] = useState("include");
  const { error, isLoading, data, mutate, isSuccess } = useUpdateSubscription();
  ...
  const [selectedMerchant, setSelectedMerchant] = useState<any>(currentId);
  ...
  const handleLogin = (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    if (selectedMerchant?.mobile_no?.length == 10) {
      mutate({
        ...
        GST_No: selectedMerchant?.GST_No as string,
  ...
  useEffect(() => {
    //@ts-ignore
    setPlan(selectedFlavors[0]?.label);
  ...
  }, [error, data, router, setSubTab, selectedFlavors, isSuccess]);
  ...
  }, [GSTTyperefs, selectGST, selectedGSTValue]);
  ...
  useEffect(() => {
    ...
    const excludeData = totalPrice1 * 0.01 * parseInt(selectedGSTValue);
        ...
        <div className={styles.inputFlex}>
          <div className={styles.inputbox}>
            <input
              ...
              defaultValue={currentId?.vendors_name}
              name="service"
          ...
          <div>
            <div className={styles.inputbox}>
              <input
                ...
                name="price"
                disabled
                value={selectedMerchant?.price}
                onChange={(e) => setPrice(totalPrice1.toString())}
              />
